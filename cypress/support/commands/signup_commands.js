...
Cypress.Commands.add('validarBotao', (text) => {
    cy.get(HEADER_PAGE.SIG_UP.BTN_SIGNUP).should('be.visible').should('contain.text', text).click()
})
Cypress.Commands.add('validarModalSignUp', (text) => {
    cy.get(HEADER_PAGE.SIG_UP.H5_SIGNUP).should('be.visible').should('contain.text', text).click()
})
